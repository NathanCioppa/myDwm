...
drawbar(Monitor *m)
{
	...
	if (m == selmon) { /* status is only drawn on selected monitor */
		...
		tw = TEXTW(stext) - lrpad; /*use lrpad + 2 for 2px right padding */
...
// windows spawned from the master terminal should not be placed in the same tag as it
...
void
cycleTagsLeft(const Arg *arg){
	unsigned int tagmask = selmon->tagset[selmon->seltags];
	unsigned int shouldSelectLast = tagmask & 1;
	tagmask = tagmask >> 1;
	if(shouldSelectLast){
		tagmask = tagmask | (1 << (LENGTH(tags)-1));
	}
	tagmask = tagmask & TAGMASK;
	Arg a = {.ui = tagmask};
	view(&a);
}

void
cycleTagsRight(const Arg *arg){
	unsigned int tagmask = selmon->tagset[selmon->seltags];
	unsigned int shouldSelectFirst = tagmask & (1<<(LENGTH(tags)-1));
	tagmask = tagmask << 1;
...
void
handleMasterTerminalRedirect(Client *c){
	if(!(mons->next)) {
        c->tags = (1 << 1);
        Arg a = {.ui = 1 << 1};
        view(&a);
